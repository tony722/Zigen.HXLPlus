...
namespace AET.Zigen.HxlPlus.Tests {
  ...
  public class AudioSettingsTests {
    ...
    public void Poll_SendsCorrectJson() {
      var api = Test.HxlPlus.AllAudioSettings[11];
      api.Output.Should().Be(12);
      api.Poll();
      TestHttpClient.Url.Should().Be("http://test/GetAudioSettings");
      TestHttpClient.RequestContents.Should().Be(@"{""output"":11}");
    }
    ...
    private void TestScaledValue(string name, string jsonName, ushort output, ushort outputScaled) {
      ...
      TestHttpClient.RequestContents.Should().Be(String.Format(@"{{""output"":3,""{0}"":{1}}}", jsonName, outputScaled));
    }
    ...
    private void TestEq(string name, string jsonName) {
      ...
      hxl.SetPropertyValue<SetShortOutputDelegate>("Set" + name + "F", v => hxlState = v);
      hxl.SetPropertyValue<SetStringOutputDelegate>("Set" + name + "Text", v => hxlText = v.ToString());
      ...
      using (new AssertionScope()) {
        ...
        state.Should().Be(30);
        text.Should().Be("3");
        hxlState.Should().Be(state);
        hxlText.Should().Be(text);
        TestHttpClient.RequestContents.Should().Be(String.Format(@"{{""output"":3,""{0}"":3}}", jsonName));
        ...
        hxlState.Should().Be(state);
        hxlText.Should().Be(text);
        TestHttpClient.RequestContents.Should().Be(String.Format(@"{{""output"":3,""{0}"":-6}}", jsonName));
      }
    }
    ...
    private void TestString(string name, string jsonName, string jsonValue) {
      ...
      state.Should().Be(1);
      TestHttpClient.RequestContents.Should().Be(string.Format(@"{{""output"":3,""{0}"":""{1}""}}", jsonName, jsonValue));
    }

    private void TestInts(string[] names, string jsonName, int[] jsonValues) {
      TestValues(names, jsonName, jsonValues.Select(a => (object)a).ToArray(), @"{{""output"":3,""{0}"":{1}}}");
    }
    private void TestStrings(string[] names, string jsonName, string[] jsonValues) {
      TestValues(names, jsonName, jsonValues, @"{{""output"":3,""{0}"":""{1}""}}");
    }
